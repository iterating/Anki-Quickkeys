...
while (true);do
	...
	if (( $difftime > 200 )); then
		if [ "${check}" == "ff nw" ] && [ "${lastcheck}" != 1 ]; then
			...
			perl -pi -e "s/[^\n]\s{3}(K.*)(# Firefox-nw)/    # \$1\$2/g;s/[^\n]\s{3}#\s(K.*)(# Beginning of Line)/    \$1\$2/g;s/[^\n]\s{3}#\s(K.*)(# End of Line)/    \$1\$2/g;s/[^\n]\s{3}(K.*)(# Chrome-nw)/    # \$1\$2/g" /tmp/kinto/xkeysnail/kinto-multikey.py 2>/dev/null
			# cp /home/ryan/.config/kinto/kinto-multikey.py /tmp/kinto/xkeysnail/kinto-multikey.py
			lastcheck=3
		fi
	fi
done
