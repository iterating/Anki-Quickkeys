...
echo "0" > /tmp/ankikeys/caret

millitime=`date +%s%3N`
echo "$millitime" > /tmp/ankikeys/millitime
...
while (true);do
	...
	check=$(cat /tmp/ankikeys/caret)
	millitime=$(cat /tmp/ankikeys/millitime)
	newtime=`date +%s%3N`
	difftime=$(( newtime - millitime ))
	if (( $difftime > 200 )); then
		if [ "${check}" == "ff nw" ] && [ "${lastcheck}" != 1 ]; then
			...
			perl -pi -e "s/[^\n]\s{3}(K.*)(# Firefox-nw)/    # \$1\$2/g;s/[^\n]\s{3}#\s(K.*)(# Beginning of Line)/    \$1\$2/g;s/[^\n]\s{3}#\s(K.*)(# End of Line)/    \$1\$2/g;s/[^\n]\s{3}(K.*)(# Chrome-nw)/    # \$1\$2/g" /tmp/ankikeys/xkeysnail/ankikeys-multikey.py 2>/dev/null
			# cp /home/ryan/.config/ankikeys/ankikeys-multikey.py /tmp/ankikeys/xkeysnail/ankikeys-multikey.py
			lastcheck=3
		fi
	fi
done
